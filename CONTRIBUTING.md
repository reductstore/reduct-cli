...
We want ReductStore CLI to be easy to contribute to, especially for people joining the project for the first time.
...
- If you want a small first task, start with [good first issue](https://github.com/reductstore/reduct-cli/issues?q=is%3Aissue%20is%3Aopen%20label%3A%22good%20first%20issue%22).
...
Contributors should run the CLI from their branch, not only the published binary, so the command actually includes their changes.
