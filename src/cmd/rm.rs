    ...
    Command::new("rm")
        ...
        .arg(
            Arg::new("start")
                .long("start")
                .short('b')
                .help("Remove records  with timestamps older than this time point as Unix microseconds, RFC3339, local date/datetime, or relative time (e.g. now - 1h).\nIf not specified, the export will start from the first record in an entry.")
                .required(false)
        )
        .arg(
            Arg::new("stop")
        ...
        .arg(
            Arg::new("time")
                ...
                .help("Remove a record with a certain timestamp as Unix microseconds, RFC3339, local date/datetime, or relative time (e.g. now - 1h).")
                .required(false)
                .num_args(0..)
        )
}
